...
namespace HandsOnWheelDetection
{
    class HandsRecognition
    {
        ...
        private Dictionary<int, string> handStatus = new Dictionary<int, string>();

        private bool pastHandsOnWheel = false;

        private void alertHandler(PXCMHandData.AlertData data)
        {
            switch (data.label)
            {
                case PXCMHandData.AlertType.ALERT_HAND_INSIDE_BORDERS:
                    handStatus.Clear();
                    handStatus[data.handId] = "OK";
                    break;
                case PXCMHandData.AlertType.ALERT_HAND_OUT_OF_BORDERS:
                case PXCMHandData.AlertType.ALERT_HAND_NOT_TRACKED:
                case PXCMHandData.AlertType.ALERT_HAND_NOT_DETECTED:
                //case PXCMHandData.AlertType.ALERT_HAND_LOW_CONFIDENCE:
                    handStatus[data.handId] = "OUT";
                    break;
            }
            var lines = handStatus.Select(kvp => kvp.Key + ": " + kvp.Value.ToString());
            Console.WriteLine(string.Join("|", lines) + "/////// hand_" + data.handId + ":" + data.label);


            //if (handStatus.Count >= 1)
            //{
                bool currentOnWheel = false;
                foreach (string s in handStatus.Values)
                {
                    if (s == "OK") currentOnWheel = true;
                }

                // if there is a change of state, trigger mqtt message
                if ((currentOnWheel && !pastHandsOnWheel) || !currentOnWheel && pastHandsOnWheel)
                {
                    JObject head = new JObject(
                        new JProperty("Type", "OnWheel"),
                        new JProperty("State", currentOnWheel)
                    );
                    mClient.publish(head.ToString(), "contextsense/monitor/hands", false);
                }
                pastHandsOnWheel = currentOnWheel;
            //}
            //if (handStatus.Count >= 2) handStatus.Clear();
        }
        ...
        {   
            ...
            bool flag = true;
            PXCMSenseManager senseManager = null;
            _disconnected = false;
            senseManager = _form.g_session.CreateSenseManager();
            ...
            PXCMCaptureManager captureManager = senseManager.captureManager;
            ...
            if (_form.GetCursorModeState())
            {
                ...
                pxcmStatus status = senseManager.EnableHandCursor();
                handCursorAnalysis = senseManager.QueryHandCursor();
            ...
            if (senseManager.Init(handler) == pxcmStatus.PXCM_STATUS_NO_ERROR)
            {
                ...
                else
                {
                    ...
                    if (handConfiguration != null)
                    {
                        // set tracking boundaries and alerts
                        handConfiguration.SetTrackingBounds(0.30f, 0.50f, 0.2f, 0.2f);
                        ...
                        handConfiguration.EnableAlert(AlertType.ALERT_HAND_INSIDE_BORDERS);
                        handConfiguration.SubscribeAlert(alertHandler);
                ...
                while (!_form.stop)
                {
                    ...
                    if (!DisplayDeviceConnection(!senseManager.IsConnected()))
                    {
                        ...
                        if (_form.GetCursorModeState())
                        {
                            sample  = senseManager.QueryHandCursorSample();
                        }
                        else
                        {
                            sample = senseManager.QueryHandSample();
                        }
                        if (sample != null && sample.depth != null)
                        {
                            frameNumber = liveCamera ? frameCounter : senseManager.captureManager.QueryFrameIndex(); 
